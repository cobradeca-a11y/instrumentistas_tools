...
Melhorias em relação à v5.1:
  P1 — LINES_MAP eliminado: faixas de acordes e letra detectadas automaticamente
       Regra: acorde_window = [prev_y_bot ... y_top_atual]
              letra_window  = [y_bot ... proximo_y_top]
              sistema com letra → extrai acordes; sem letra → pula
  P2 — Beat melhorado: calculado pela nota âncora relativa às barras,
       não pela posição proporcional do acorde
  P3 — Fontes expandidas: aceita OpusStd e Maestro para notas musicais
  P4 — Seções lineares: o pipeline não nomeia seções; o editor faz isso
"""
...
FIGURE_BASE = {
    ...
    '˙': ('mínima',          2.0, False),
    'ú': ('mínima',          2.0, False),   # Maestro usa 'ú'
...
# Textos a ignorar na extração de letra
SKIP_TEXTS = {
    'Coro','coro','(Ageu)','Ageu 2.6-9',
    'Arr. Vocal:','Arr. Cordas:','Arr. de Cordas:',
    'To Coda','D.S. al Coda','D.C. al Coda','Fine',
    'DEPARTAMENTO DE LOUVOR',
}
SKIP_PATTERNS = [
    r'^\d+$',                    # só número (número de página)
    r'^= \d',                    # = 75 - 85 (bpm)
    r'^q\s*=',                   # q = 80
    r'^Sérgio','r^Luciana',
    r'^Francisco',
...
def should_skip(text):
    ...
    if text in SKIP_TEXTS: return True
    if any(text.startswith(p) for p in ['Arr.','DEPART','Sérgio','Luciana',
                                          'Francisco','Felipe','Daniel','Fábio']):
        return True
    for pat in SKIP_PATTERNS:
        if re.match(pat, text): return True
    return False
...
def is_note_font(fontname):
    """Aceita OpusStd e Maestro como fontes de notação."""
    return 'OpusStd' in fontname or 'Maestro' in fontname


# ════════════════════════════════════════════════════════════════
# P1 — DETECÇÃO AUTOMÁTICA DE SISTEMAS E FAIXAS
# ════════════════════════════════════════════════════════════════

def detect_systems_auto(page):
    """
    Detecta sistemas de pauta e calcula automaticamente as janelas
    de acordes e letra para cada sistema.

    Janela de acordes: entre o y_bot do sistema anterior e o y_top do atual.
    Janela de letra:   entre o y_bot do sistema atual e o y_top do próximo.

    Retorna lista de sistemas com campos:
      y_top, y_bot, lines_y, x_esq, x_dir,
      ac_y_min, ac_y_max,   ← janela de acordes
      ly_y_min, ly_y_max    ← janela de letra
    """
    h = [l for l in page.lines
         if abs(l['y0']-l['y1']) < 0.5 and abs(l['x1']-l['x0']) > 400]
    ...
    # Calcular janelas automáticas
    page_height = page.height
    for i, sis in enumerate(sistemas):
        # Janelas calibradas empiricamente em 3 PDFs diferentes:
        # Acordes ficam até 80px ACIMA do y_top da pauta
        # Letra fica entre y_bot+2 e y_bot+150
...
def has_lyrics(sis, lyric_tokens):
    """Sistema tem letra se houver tokens Times não-skip na janela de letra."""
    return any(
...
def group_tokens(chars, x_tol=3):
    """Agrupa chars adjacentes em tokens por linha de top."""
    # Filtrar ligaduras de sílaba OpusTextStd
    ...
    for tk in sorted(ld):
        ...
        for c in sorted(ld[tk], key=lambda c: c['x0']):
            if not cur or c['x0']-cur[-1]['x1'] <= x_tol:
                ...
                if t:
                    words.append({'text':t, 'top':cur[0]['top'],
...
# ════════════════════════════════════════════════════════════════
# DETECÇÃO DE CLAVE, ARMADURA E METRO
# ════════════════════════════════════════════════════════════════

def detect_clef_and_key(chars, sis):
    ctx = [c for c in chars
    ...
    arm = [c for c in ctx if c['text'] in {'b','#'}
...
# ════════════════════════════════════════════════════════════════
# DETECÇÃO DE BARRAS DE COMPASSO
# ════════════════════════════════════════════════════════════════
...
def detect_barlines(lines, sis):
    ...
    b = [l for l in v
         ...
         and (abs(l['y0']-sis['y_top']) < 3 or abs(l['y1']-sis['y_top']) < 3)]
...
def rhythmic_figure(char, cx, top, all_chars):
    ...
    flags = len(set(round(c['cx'],0) for c in all_chars
                    ...
                    and abs(c['cx']-cx)<=6 and abs(c['top']-top)<=20))
...
# ════════════════════════════════════════════════════════════════
# P2 — BEAT MELHORADO: por nota âncora relativa às barras
# ════════════════════════════════════════════════════════════════

def calc_beat_by_anchor(nota_anchor_cx, comp, meter):
    """
    P2: Calcula beat usando a posição da nota âncora relativa ao compasso.
    Mais estável que usar o cx do acorde diretamente.
    Se não há nota âncora, usa o cx do acorde (fallback).
    """
...
# ════════════════════════════════════════════════════════════════
# DETECÇÃO DE MARCAÇÕES DE FORMA
# ════════════════════════════════════════════════════════════════
...
def detect_form_marks(lyric_tokens, opus_chars, sistemas):
    """
    Detecta marcações de forma (segno, to_coda, ds_al_coda, coda).
    Retorna lista de dicts com tipo, página, sistema e cx.
    """
    marks = []
    ...
    for t in lyric_tokens:
        for label, tipo in FORM_LABELS.items():
            if label in t['text']:
                marks.append({'type':tipo,'cx':t['cx'],'top':t['top'],'label':t['text']})

    # Segno: char '%' na OpusStd/Maestro
    for c in opus_chars:
        if c['text'] == '%':
            marks.append({'type':'segno','cx':(c['x0']+c['x1'])/2,'top':c['top'],'label':'⊕'})
...
def run(pdf_path, meta=None):
    """
    P1: Roda o pipeline sem LINES_MAP.
    Detecta automaticamente sistemas com letra e extrai acordes/letra.

    pdf_path: caminho do PDF vetorial
    meta:     dict opcional com title, composer, key, key_sig, bpm, meter

    Retorna: (output_dict, audit_list)
    """
    meta = meta or {}
    ...
    output = {
        ...
        "generated_at":     ts,
        "form":             [],
        # Seções lineares — editor nomeia depois
        "sections": [{
            "id":      "linear",
            "label":   "Linear",
            "clef":    meta.get("clef","treble"),
    ...
    audit_rows = []
    inherited  = {"meter":4,"clef":"treble","key_sig":None}
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for pg_idx in range(len(pdf.pages)):
            ...
            # Pré-calcular cx
            for c in chars:
                if is_note_font(c.get('fontname','')) or \
                   'OpusChords' in c.get('fontname','') or \
                   'Times' in c.get('fontname',''):
                    c['cx'] = (c['x0']+c['x1'])/2

            # P1: detectar sistemas com janelas automáticas
            ...
            # Marcações de forma desta página
            ...
            for sis in sistemas:
                ...
                # P1: extrair tokens nas janelas automáticas
                acordes_sis = sorted(
                ...
                # Sistema sem letra = sem acorde para cifra → pular
                # (pauta de baixo, clave de fá, instrumental)
                if not silas_sis and not acordes_sis:
                    continue

                # Detecção de clave/armadura/metro
                ...
                # Notas apenas da voz soprano (top < linha3)
                opus_sis = [c for c in opus_chars
                ...
                for comp in compassos:
                    ...
                    for ac in ac_in:
                        ...
                        audit_rows.append({
                            'pg':pg_idx,'sis':sis['id'],'comp_n':measure_global,
                    ...
                    for c in notas_comp_map.get(comp['n'],[]):
                        ...
                        if not fig['is_rest']: m_obj['beats_used']+=fig['beats']
...
if __name__ == "__main__":
    ...
    configs = [
        ('/mnt/user-data/uploads/Ainda_Uma_Vez_-_Coro.pdf',
         {'title':'Ainda Uma Vez','composer':'Francisco Jose Barbosa Condi',
          'key':'Cm','key_sig':-3,'bpm':'75-85'}),
        ('/mnt/user-data/uploads/Cordeiro_Eterno_-_Coro.pdf',
         {'title':'Cordeiro Eterno','composer':'Sérgio Paulo Ferreira de Matos',
          'key':'E','key_sig':4,'bpm':'80-90'}),
        ('/mnt/user-data/uploads/Quando_Buscamos_-_Coro.pdf',
         {'title':'Quando Buscamos','composer':'Luciana Monteiro Carneiro Simora',
          'key':'D','key_sig':2,'bpm':'60-70'}),
    ]
    ...
    for pdf_path, meta in configs:
        ...
        # Estatísticas
        total_comp = sum(len(l['measures'])
                         for s in output['sections']
                         for l in s['lines'])
        total_ch   = sum(len(m['chords'])
                         for s in output['sections']
                         for l in s['lines']
                         for m in l['measures'])
        total_syl  = sum(len(m['syllables'])
                         for s in output['sections']
                         for l in s['lines']
                         for m in l['measures'])
        by_rule = Counter(r['regra'] for r in audit)
