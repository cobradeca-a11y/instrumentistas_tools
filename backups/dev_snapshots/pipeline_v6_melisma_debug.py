...
P7 — Limpeza leve de sílabas:
     - remove marcador de verso/aspas
     - não altera janela de letra
     - não altera detecção de acordes
P8 — Debug de melisma:
...
def clean_lyric_text(text):
    """
    Limpeza leve de sílaba sem alterar janelas nem detecção de acordes.
    Remove apenas marcações visuais de verso/aspas/pontuação final.
    """
    if text is None:
        return ""
    ...
    # remove marcador de verso: 1."A- / 2."Mi / 1. A-
    t = re.sub(r'^\d+\.\s*["“”]?', '', t)

    # remove aspas externas
    t = t.strip('"“”')

    # remove aspas/ponto finais que entram grudados em sílabas
    t = re.sub(r'["“”]+$', '', t)
    t = re.sub(r'\.$', '', t)
...
def group_tokens(chars, x_tol=3, clean_lyrics=False):
    chars = [
        c for c in chars
...
def pick_syllable_for_chord(chord_cx, anchor_cx, silas_c, max_left=8, max_right=42):
    ...
    valid = [s for s in silas_c if not should_skip(s.get('text', ''))]
    ...
    if window:
        ...
        if rightish:
            return min(rightish, key=lambda s: (abs(s['cx'] - ref_cx), s['cx']))

        return min(window, key=lambda s: (abs(s['cx'] - ref_cx), s['cx']))

    after = [s for s in valid if s['cx'] >= ref_cx - max_left]

    if after:
        return min(after, key=lambda s: s['cx'])

    before = [s for s in valid if s['cx'] < ref_cx]

    if before:
        return max(before, key=lambda s: s['cx'])

    return min(valid, key=lambda s: abs(s['cx'] - ref_cx))
...
def t90_full(chord_cx, compassos, notas_comp_map, silas_por_comp,
    ...
    if nota_anchor and silas_c:
        if is_probable_melisma(nota_anchor['cx'], silas_c):
            melisma_candidates_same_measure = [
                s for s in silas_c
                if s['cx'] < nota_anchor['cx'] - 5
            ]

            melisma_candidates_prev_measures = []
            melisma_prev_comp_found = None

            silas_antes = list(melisma_candidates_same_measure)
            ...
            if not silas_antes:
                for pn in range(comp['n'] - 1, 0, -1):
                    ...
                    if p:
                        melisma_candidates_prev_measures = p
                        melisma_prev_comp_found = pn
                        silas_antes = p
                        break

            melisma_candidates_prev_line = []
            if silas_linha_anterior:
                melisma_candidates_prev_line = silas_linha_anterior

            used_source = None

            if melisma_candidates_same_measure:
                used_source = "same_measure"
            elif melisma_candidates_prev_measures:
                used_source = f"previous_measure_{melisma_prev_comp_found}"
            elif not silas_antes and melisma_candidates_prev_line:
            ...
            if target:
                ...
                reason = "no_previous_syllable_available"
            ...
            return {
                ...
                "rule": "T90.2_MELISMA",
                "melisma_debug": {
                    "anchor_cx": round(nota_anchor['cx'], 1) if nota_anchor else None,
                    "current_measure": comp['n'],
                    "used_source": used_source,
                    "reason": reason,
                    "same_measure_candidates": [
                        {
                            "text": s.get("text"),
                            "cx": round(s.get("cx", 0), 1),
                            "top": round(s.get("top", 0), 1),
                        }
                        for s in melisma_candidates_same_measure
                    ],
                    "previous_measure": melisma_prev_comp_found,
                    "previous_measure_candidates": [
                        {
                            "text": s.get("text"),
                            "cx": round(s.get("cx", 0), 1),
                            "top": round(s.get("top", 0), 1),
                        }
                        for s in melisma_candidates_prev_measures
                    ],
                    "previous_line_candidates": [
                        ...
                        for s in melisma_candidates_prev_line
                    ],
                },
            }
...
def run(pdf_path, meta=None):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for pg_idx, page in enumerate(pdf.pages):
            ...
            lyric_tokens = group_tokens(lyric_chars, clean_lyrics=True)
                ...
                for comp in compassos:
                    ...
                    for ac in ac_in:
                        ...
                        protocol_block = {
                            ...
                            "melisma_debug": r.get('melisma_debug'),
                        }
