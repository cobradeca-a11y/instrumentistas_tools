#!/usr/bin/env python3
"""
calibrate.py — Mesclagem, coverage, review e MergeGuard2 do protocolo CXD+T90.
...
def _should_force_presence_only(chord, match, match_method):
    """
    MergeGuard2.
    Mantém coverage estrutural, mas só força presence_only quando o match é fraco de verdade.

    Permite validação quando:
      - n_sym_ratio / near_measure_ratio
      - beat_delta == 0
      - ratio_delta <= 0.10
      - não é VAZIO vazio-vazio
    """
    if not match:
        return False
    ...
    if method not in guard_methods:
        return False
    ...
    ratio_delta, bi_delta, beat_delta = _score_match(chord, match)
    ...
    melisma_debug = protocol.get('melisma_debug') or {}
    if rule == 'T90.2_MELISMA':
        if melisma_debug.get('reason') == 'no_previous_syllable_available' and h_syl:
            return True

    # Match por sequência com sílaba divergente é fraco por natureza.
    if method in {'seq_align', 'seq_ratio'} and p_syl != h_syl:
        return True

    # near_measure_loose nunca valida sílaba divergente.
    if method == 'near_measure_loose' and p_syl != h_syl:
        return True

    # Permite validação forte mesmo com sílaba divergente aparente:
    # mesmo compasso/símbolo aproximado + beat perfeito + ratio muito próximo.
    if method in {'n_sym_ratio', 'near_measure_ratio'}:
        if beat_delta == 0 and ratio_delta <= 0.10:
            return False

    # Divergência horizontal forte: estrutural apenas.
    if p_syl != h_syl:
        p_cx = protocol.get('syllable_cx')
        if p_cx is None:
            p_cx = protocol.get('float_cx')

        h_cx = e_human.get('syllable_cx')
        if h_cx is None:
            h_cx = e_human.get('cx_display')

        p_cx = _safe_float(p_cx, None)
        h_cx = _safe_float(h_cx, None)

        if p_cx is not None and h_cx is not None:
            if abs(p_cx - h_cx) >= 18:
                return True
